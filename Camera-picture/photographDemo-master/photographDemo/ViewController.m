...
@property (nonatomic)UIButton *PhotoButton; //拍照按钮
...
- (void)customUI{
    ...
    [_PhotoButton setImage:[UIImage imageNamed:@"photograph_Select"] forState:UIControlStateSelected];
    ...
    _flashButton.backgroundColor = [UIColor lightGrayColor];
...
- (void)changeCamera{
    ...
    if (cameraCount > 1) {
        ...
        animation.type = @"oglFlip";//动画效果：上下翻转效果
...
- (void)focusAtPoint:(CGPoint)point{
    ...
    if ([self.device lockForConfiguration:&error]) {
        ...
        // AVCaptureFocusModeAutoFocus焦距模式调整
        if ([self.device isFocusModeSupported:AVCaptureFocusModeAutoFocus]) {
            [self.device setFocusPointOfInterest:focusPoint];
            [self.device setFocusMode:AVCaptureFocusModeAutoFocus];
        }
        
        //AVCaptureExposureModeAutoExpose 曝光量调节
